...
export default function Dashboard() {
  ...
  const totalLoans = LOANS.filter((l) => l.status === "Active").reduce((a, l) => a + l.balance, 0);
  const flagged = TRANSACTIONS.filter((t) => t.flagged).length;

  return (
    <Page
      title={`Welcome back, ${user.name.split(" ")[0]} 👋`}
      subtitle="Here is what's happening across your cooperative today."
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={PiggyBank} label="Total Savings" value={money(totalSavings)} trend="+4.2% MoM" tone="green" />
        <StatCard icon={Landmark} label="Outstanding Loans" value={money(totalLoans)} trend="3 active" tone="violet" />
        <StatCard icon={AlertTriangle} label="Flagged Transactions" value={flagged} trend="Last 24h" tone="red" />
      </div>
